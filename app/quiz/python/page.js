...
import { pythonQuestions } from "@/Components/Q-A";
...
function QPython() {
  ...
  const handleOptionClick = (option) => {
    ...
    const isAnswerCorrect = pythonQuestions[index].answer === option;
    setIsCorrect(isAnswerCorrect);
    if (option == pythonQuestions[index].answer) {
      ...
      <div className="quiz none">
        ...
        <div className=" w-[500px]">
          <p className="question text-3xl text-center mb-10">
            Q{index + 1}: {pythonQuestions[index].question}
          </p>
          {pythonQuestions[index].options.map((option) => {
