...
import { cssQuestions } from "@/Components/Q-A";
...
function QCss() {
  ...
  const handleOptionClick = (option) => {
    ...
    const isAnswerCorrect = cssQuestions[index].answer === option;
    setIsCorrect(isAnswerCorrect);
    if (option == cssQuestions[index].answer) {
      ...
      <div className="quiz none">
        ...
        <div className=" w-[500px]">
          <p className="question text-3xl text-center mb-10">
            Q{index + 1}: {cssQuestions[index].question}
          </p>
          {cssQuestions[index].options.map((option) => {
            ...
            return (
              <div
                key={option}
                className={`${style.option} option border-solid border-2 border-black rounded-full py-2 px-6 
                ${optionClass} mt-2 mb-2 text-2xl cursor-pointer`}
