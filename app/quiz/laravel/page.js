...
import { laravelQuestions } from "@/Components/Q-A";
...
function QLaravel() {
  ...
  const handleOptionClick = (option) => {
    ...
    const isAnswerCorrect = laravelQuestions[index].answer === option;
    setIsCorrect(isAnswerCorrect);
    if (option == laravelQuestions[index].answer) {
    ...
    setTimeout(() => {
      ...
      setIsCorrect(null);
      if (index < laravelQuestions.length - 1) {
          setIndex(index + 1);
      } else {
    ...
    <div className="relative h-screen w-screen grid place-content-center font-mono bg-slate-300">
      ...
      <div className="quiz none">
        <h1 className="text-5xl font-medium text-center mb-16">
          Welcome to Laravel Quiz
        </h1>
        <div className=" w-[500px]">
          <p className="question text-3xl text-center mb-10">
            Q{index + 1}: {laravelQuestions[index].question}
          </p>
          {laravelQuestions[index].options.map((option) => {
