...
import { nodejsQuestions } from "@/Components/Q-A";
...
function QNode() {
  ...
  const handleOptionClick = (option) => {
    ...
    const isAnswerCorrect = nodejsQuestions[index].answer === option;
    setIsCorrect(isAnswerCorrect);
    if (option == nodejsQuestions[index].answer) {
    ...
    setTimeout(() => {
      ...
      setIsCorrect(null);
      if (index < nodejsQuestions.length - 1) {
          setIndex(index + 1);
      } else {
    ...
    <div className="relative h-screen w-screen grid place-content-center font-mono bg-slate-300">
      ...
      <div className="quiz none">
        <h1 className="text-5xl font-medium text-center mb-16">
          Welcome to Node JS Quiz
        </h1>
        <div className=" w-[500px]">
          <p className="question text-3xl text-center mb-10">
            Q{index + 1}: {nodejsQuestions[index].question}
          </p>
          {nodejsQuestions[index].options.map((option) => {
