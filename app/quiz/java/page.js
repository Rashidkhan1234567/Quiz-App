...
import { javaQuestions } from "@/Components/Q-A";
...
function QJava() {
  ...
  const handleOptionClick = (option) => {
    ...
    const isAnswerCorrect = javaQuestions[index].answer === option;
    setIsCorrect(isAnswerCorrect);
    if (option == javaQuestions[index].answer) {
    ...
    setTimeout(() => {
      ...
      setIsCorrect(null);
      if (index < javaQuestions.length - 1) {
          setIndex(index + 1);
      } else {
      ...
      <div className="quiz none">
        ...
        <div className=" w-[500px]">
          <p className="question text-3xl text-center mb-10">
            Q{index + 1}: {javaQuestions[index].question}
          </p>
          {javaQuestions[index].options.map((option) => {
