...
import { typescriptQuestions } from "@/Components/Q-A";
import { FaRegHandPointLeft } from "react-icons/fa";
import style from "@/Style/Mix.module.css"

function QTypescript() {
  ...
  const handleOptionClick = (option) => {
    ...
    const isAnswerCorrect = typescriptQuestions[index].answer === option;
    setIsCorrect(isAnswerCorrect);
    if (option == typescriptQuestions[index].answer) {
    ...
    setTimeout(() => {
      ...
      setIsCorrect(null);
      if (index < typescriptQuestions.length - 1) {
          setIndex(index + 1);
      } else {
      ...
      <div className="quiz">
        ...
        <div className=" w-[500px]">
          <p className="question text-3xl text-center mb-10">
            Q{index + 1}: {typescriptQuestions[index].question}
          </p>
          {typescriptQuestions[index].options.map((option) => {
