...
import { phpQuestions } from "@/Components/Q-A";
...
function QPhp() {
  ...
  const handleOptionClick = (option) => {
    ...
    const isAnswerCorrect = phpQuestions[index].answer === option;
    setIsCorrect(isAnswerCorrect);
    if (option == phpQuestions[index].answer) {
    ...
    setTimeout(() => {
      ...
      setIsCorrect(null);
      if (index < phpQuestions.length - 1) {
          setIndex(index + 1);
      } else {
      ...
      <div className="quiz none">
        ...
        <div className=" w-[500px]">
          <p className="question text-3xl text-center mb-10">
            Q{index + 1}: {phpQuestions[index].question}
          </p>
          {phpQuestions[index].options.map((option) => {
