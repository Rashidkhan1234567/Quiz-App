...
import { htmlQuestions } from "@/Components/Q-A";
...
function Qhtml() {
  ...
  const handleOptionClick = (option) => {
    ...
    const isAnswerCorrect = htmlQuestions[index].answer === option;
    setIsCorrect(isAnswerCorrect);
    if (option == htmlQuestions[index].answer) {
      ...
      <div className="quiz none">
        ...
        <div className=" w-[500px]">
          <p className="question text-3xl text-center mb-10">
            Q{index + 1}: {htmlQuestions[index].question}
          </p>
          {htmlQuestions[index].options.map((option) => {
