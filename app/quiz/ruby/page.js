...
import { rubyQuestions } from "@/Components/Q-A";
...
function QRuby() {
  ...
  const handleOptionClick = (option) => {
    ...
    const isAnswerCorrect = rubyQuestions[index].answer === option;
    setIsCorrect(isAnswerCorrect);
    if (option == rubyQuestions[index].answer) {
    ...
    setTimeout(() => {
      ...
      setIsCorrect(null);
      if (index < rubyQuestions.length - 1) {
          setIndex(index + 1);
      } else {
      ...
      <div className="quiz none">
        ...
        <div className=" w-[500px]">
          <p className="question text-3xl text-center mb-10">
            Q{index + 1}: {rubyQuestions[index].question}
          </p>
          {rubyQuestions[index].options.map((option) => {
